...
**P.S. Ошибки при удалении ветки:**

* `"error: branch 'testDelet' not found."` - возникает в случае ввода имени ветки с ошибкой;
* `"error: The branch 'testDelete' is not fully merged."` - в случае, если Вы __*НЕ выполнили*__ слияние удаляемой ветки с какой-либо из веток.

Если содержимое ветки не было перенесено ни в какую другую ветку, оно не нужно, возникает ошибка `"error: The branch 'testDelete' is not fully merged."`, но ветку всё равно нужно удалить, следует ввести команду:

```
git branch -D <имя-ветки>
```

После ввода команды, можно убедиться в её выполнении, вызвав список существующих веток знакомой командой: `git branch.` Ветка, имя которой было прописано в команде на удаление, соответственно, должно будет отсутствовать в списке.

## 5. Работа с удалёнными репозиториями в Git
___

Для начала уточним, что сам по себе `Git` является программой, установленной на Вашем компьютере и внутри него __*локально*__ позволяет работать с репозиториями.

Для __*удаленной*__ работы существуют специальные сервисы, позволяющие интегрироваться с программий `Git` на Вашем компьютере и настроить удаленную работу с репозиториями.

Рассмотрим на примере сервиса `GitHub` компании Microsoft. 

Он предоставляет доступ к множеству проектов (репозиториев), в которых Вам, возможно, захочется поучаствовать. Репозитории с **открытым доступом** помечены отметкой `Public`.

P.S.: Для того чтобы разместить на `GitHub` свой репозиторий, необходимо создать и зарегистрировать свой аккаунт.

### 5.1 Основные команды

* ### 5.1.1 Клонирование удалённого репозитория

  * Создаём на рабочем столе новую папку, открываем её в редакторе кода, но не инициализируем локальный репозиторий, как делали это раньше.

  * Вместо этого на `GitHub` выбираем интересующий готовый репозиторий с открытым доступом, нажимаем на зелёную кнопочку **Code**, после чего появляется строка с адресом в формате Https, копируем эту ссылку.

  * В терминале редактора вводим команду:

  ```
  git clone <скопированный адрес>
  ```
  После этого внутри Вашей папки появится новая папка с названием репозитория, который Вы выбрали на `GitHub`, то есть полная её копия, со всеми вложенными файлами.

  При попытке проверить статус командой `git status`, появится сообщение об ошибке. Это происходит потому, что данная команда выполняется в созданной Вами папке на рабочем столе, а в ней нет репозитория, для того чтобы перейти в папку-репозиторий, скопированную с `GitHub`, необходимо сменить директорию командой: 
  
  ```
  cd <название папки-репозитория, скопированного с GitHub>
  ```
  cd - означает change directory.

  После чего можно продолжать работу с файлами в этой папке уже известными командами, например `git log` чтобы просмотреть историю коммитов, увидеть этапы работы над данным файлом.

  Все изменения, произведённые Вами, после копирования удаленного репозитория на свой компьютер, будут храниться в вашем уже локальном репозитории и никак не отразятся в репозитории на `GitHub`.

* ### 5.1.2 Отправить изменения в удалённый репозиторий.

После того как с файлом на локальном репозитории была проделана какая-либо работа, сохранены изменения, написаны коммиты, отправить эти изменения на удалённый репозиторий можно с помощью команды:

  ```
  git push
  ```
  
  push - означает "толкать".

  При обновлении страницы удаленного репозитория, изменения должны будут в нем отобразиться.

  **P.S.:** Отправлять изменения командой `git push` можно только в репозитории своего аккаунта. Перед первой отправкой будет необходимо "подружить" локальный и удаленный репозитории.

  Для этого нужно в своём аккаунте на `GitHub` создать новый репозиторий, присвоить ему какое-то имя, настроить параметры и нажать внизу страницы "Create repository".

  GitHub предложит несколько вариантов и под каждым перечислит, что необходимо сделать:

  1. Создать репозиторий через терминал и начать с ним работать;

  2. Привязать к этому репозиторию уже существующий локальный репозиторий;

  3. Импортировать код из другого репозитория.

* ### 5.1.3 Скачать изменения из удалённого репозитория.

В ситуации, когда версия файла удалённого репозитория является более актуальной, чем локального, загрузить их в репозиторий своего компьютера можно командой:

  ```
  git pull
  ```
  
  pull - означает "тянуть".

  Эта команда забирает изменения и сразу проводит слияние с активной веткой.

* ### 5.1.4 Копирование удалённого репозитория.

Если Вас заинтересовал чей-то проект и Вы хотите принять участие в его разработке, но у вас нет доступа к внесению изменений, необходимо выполнить следующие дествия:

* Копирование чужого удаленного репозитория в свой удалённый репозиторий выполняется нажатием кнопки `Fork` на странице интересующего репозитория на чьём-либо аккаунте.

  Fork в переводе с английского означает "вилка", то есть некое создание ответвления.

  После чего на странице уже Вашего аккаунта появится полная копия заинтересовавшего Вас репозитория.

  Далее, уже известными командами можно произвести клонирование этого удаленного репозитория на свой локальный и приступать к работе с ним.

* При работе с чужими файлами, принято создавать отдельную ветку и файл описания с именем `README`, файл обязательно сохраняется и коммитится.

* После внесения изменений необходимо отправить их в свой удаленный репозиторий командой `git push`.

  Далее можно убедиться, что изменения прошли, новая ветка появилась - обновить страницу с репозиторием.

  И на странице появилась новая кнопочка `Compare & pull request`

  Compare - означает "Сравни"

  Pull request - отправь запрос на вливание изменений в основной репозиторий.

  Основным является тот, с которого Вы делали копию кнопкой `Fork`.

  После нажатия кнопки `Compare & pull request`, GitHub анализирует изменения на возможность бесконфликтного слияния новой ветки с основным репозиторием, предлагает оставить комментарий и нажать на кнопку `Create pull request`, то есть отправить запрос.

  Владелец аккаунта увидит уведомление о поступлении Вашего `pull request`, сможет ознакомиться и принять решение о необходимости вливания ваших изменений в свой проект.